...
from SM import SM
from Static_load import Static_load
from Electrical_system import Electrical_system
from AM import AM
from KZ_Bus import KZ_Bus
from KZ_single_phase import KZ_single_phase
from Simple_electrical_line import Simple_electrical_line

def create_image_for_menu(pass_obj):
    buff_image = ImageTk.PhotoImage(Image.open(pass_obj))
    width_image = buff_image.width()/7
...
mass_model.append([]) #mass_Simple_electrical_line
...
def graphics(w_index, num):
    ...
    if (num == 4):
        ...
        plt.plot(t, ww[:, 0 + w_index], linewidth=1, color='red')
        plt.xlabel('Время, с', fontsize=12, color='black')
        plt.ylabel('Ток фазы А системы, А', fontsize=12, color='black')
        plt.grid(True)
    ...
    if (num == 8):
        I_pr = []
        for i in range(len(t)):
            I_pr.append(-ww[:, 0 + w_index][i]/120)
        plt.figure(n_fig)
        plt.subplot(1, 1, 1)
        n_fig += 1
        plt.plot(t, ww[:, 0 + w_index], linewidth=1, color='red')
        plt.xlabel('Время, с', fontsize=12, color='black')
        plt.ylabel('Ток фазы А линии, А', fontsize=12, color='black')

        plt.figure(n_fig)
        plt.subplot(1, 1, 1)
        n_fig += 1
        plt.plot(t, ww[:, 2 + w_index], linewidth=1, color='red')
        plt.plot(t, I_pr, linewidth=1, color='black', linestyle = '--')
        plt.xlabel('Время, с', fontsize=12, color='black')
        plt.ylabel('Токи ТТ, А', fontsize=12, color='black')
        plt.grid(True)
    plt.show()
                    ...
                    if (mass_model[m][i].view_result(event.x, event.y) == True):
                        ...
                        for j in help_matrix:
                            if ((j[0] == m) and (j[1] == i)):
                                ...
                                w_index += mass_model[m][i].width_input
                        graphics(w_index, m)
...
def start(event):
    ...
    ww = odeint(f, y0, t)
    plyer.notification.notify(message='Все посчиталось, заходи в программу!',
    app_name='Modeling programm',
    title='Modeling is done', )
...
def add_Simple_electrical_line(event):
    global state_enter_parameter
    if (state_enter_parameter == 0):
        mass_model[8].append(Simple_electrical_line(mouse_x, mouse_y, canv, root))
    ...
    for i in range(len(mass_model)):
        ...
        for j in range(len(mass_model[0])):
            if (mass_model[0][j] != 0):
                ...
                for k in range(len(mass_iteraion)):
                    if (mass_iteraion[k] != "none"):
                        width_matrix = mass_model[i][k].width_matrix
                        height_matrix = mass_model[i][k].height_matrix
                        width_input = mass_model[i][k].width_input
    ...
    for i in range(len(mass_model)):
        ...
        for j in range(len(mass_model[0])):
            if (mass_model[0][j] != 0):
                ...
                for k in range(len(mass_iteraion)):
                    if (mass_iteraion[k] != "none"):                       
                        for m in range(mass_model[i][k].width_matrix):
                            ouput_matrix.append(solve_matrix[qwe_i])
                            qwe_i += 1
                        for m in range(mass_model[i][k].width_input - mass_model[i][k].width_matrix):
                            ouput_matrix.append(mass_model[i][k].get_additional_variable(y[wait_index:(wait_index+mass_model[i][k].width_input)], t)[m])
                        wait_index += mass_model[i][k].width_input  
...
root.bind('9', add_Simple_electrical_line)
canv.pack() 
root.mainloop()